    ...
    public class DBManager
    {
        ...
        public Boolean InsertRecordToTable(ArticleItem recordArticle)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = String.Format("SELECT COUNT(*) FROM articleBrowseRecordUser{0}", UserId);
                    using (SqliteDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                    ...
                    cmd.CommandText = String.Format("INSERT INTO articleBrowseRecordUser{0} (articleTitle, articleContent, articleIconPath, articleLink) VALUES(@articleTitle, @articleContent, @articleIconPath, @articleLink);SELECT last_insert_rowid();", UserId);
        ...
        public void UpdateDirectoryToTable(Int32 directoryIndex, String coverTitle, String imageName)
        {
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                {
                    cmd.Transaction = conn.BeginTransaction();
                    cmd.CommandText = String.Format("UPDATE directoryTableUser{0} SET directoryName=@directoryName WHERE id={1}", UserId, directoryIndex);
                    ...
                    {
                        cmd.Transaction = conn.BeginTransaction();
                        cmd.CommandText = String.Format("UPDATE directoryTableUser{0} SET imagePath=@imagePath WHERE id={1}", UserId, directoryIndex);
        ...
        public void InsertArticleToTable(Int32 directoryIndex)
        {
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                {
                    cmd.Transaction = conn.BeginTransaction();
                    cmd.CommandText = String.Format("INSERT INTO directoryArticlesUser{0} (directoryId, articleTitle, articleContent, articleIconPath, articleLink) VALUES(@directoryId, @articleTitle, @articleContent, @articleIconPath, @articleLink);SELECT last_insert_rowid();", UserId);
        ...
        public void DeleteDirectoryFromTable(Int32 directoryIndex)
        {
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = String.Format("DELETE FROM directoryArticlesUser{0} WHERE directoryId={1}", UserId, directoryIndex);
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = String.Format("DELETE FROM directoryTableUser{0} WHERE id={1}", UserId, directoryIndex);
                    cmd.ExecuteNonQuery();
                }
            }
        }
        ...
        public void DeleteArticleFromTable(Int32 directoryIndex)
        {
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = String.Format("DELETE FROM directoryArticlesUser{0} WHERE directoryId={1} AND articleTitle=@articleTitle", UserId, directoryIndex);
                    cmd.Parameters.Add("@articleTitle", null);
        ...
        public ObservableCollection<ArticleItem> LoadDirectoryArticlesFromTable(Int32 directoryIndex)
        {
            ...
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    ...
                    querySrting = String.Format("SELECT * FROM directoryArticlesUser{0} WHERE directoryId={1}", UserId, directoryIndex);
        ...
        public List<ArticleItem> LoadAllArticlesFromTable()
        {
            ...
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                {    //Do not search external articles
                    cmd.CommandText = String.Format("SELECT * FROM directoryArticlesUser{0} WHERE directoryId>1", UserId);
                    using (SqliteDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
        ...
        public Boolean IsTableHaveArticles(String tableName, Int32 directoryIndex)
        {
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    if (directoryIndex != -1)
                    {
                        cmd.CommandText = String.Format("SELECT * FROM {0}{1} WHERE directoryId={2}", tableName, UserId, directoryIndex);
                    }
                    else
                    {
                        cmd.CommandText = String.Format("SELECT * FROM {0}{1}", tableName, UserId);
                    }
        ...
        public ObservableCollection<MySelectedArticleDirectory> LoadDirectoriesFromTable()
        {
            ...
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                using (SqliteCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = String.Format("SELECT * FROM directoryTableUser{0}", UserId);
                    using (SqliteDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
        ...
        public void UpdateUserSettings()
        {
            using (SqliteConnection conn = new SqliteConnection("Version=3,uri=file:KKBOX_NEWS.db"))
            {
                ...
                {
                    cmd.Transaction = conn.BeginTransaction();
                    cmd.CommandText = String.Format("UPDATE userAccount SET openExternalWeb=@openExternalWeb, openAutoUpdate=@openAutoUpdate, updateInterval=@updateInterval WHERE id={0}", UserId);
