...
namespace KKBOX_News.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        void OnDownloadStringCompleted(object sender, DownloadStringCompletedEventArgs args)
        {
            ...
            TopicsInXml = ParsedTopic.Element("kkbox_news").Elements("topic");
            LoadData();
        ...
        public void LoadData()
        {
            // 範例資料; 以真實資料取代
            //SelectTopicParser();
            foreach (XElement topic in TopicsInXml)
            {
                this.Items.Add(new ItemViewModel() { LineOne = topic.Element("title").Value, LineTwo = topic.Element("icon").Value, LineThree = topic.Element("url").Value });
            }
